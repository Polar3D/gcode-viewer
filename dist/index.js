...
__export(index_exports, {
  ...
  PATH_TYPE_COLORS: () => PATH_TYPE_COLORS,
...
var GCodeViewer = class {
  ...
  applyPathTypeFilter() {
    this.layers.forEach((layer) => {
      layer.object.traverse((child) => {
        if ((child instanceof THREE.Mesh || child instanceof import_Line2.Line2) && child.userData["pathType"]) {
      ...
      this.layers.forEach((layer, index) => {
        ...
        layer.object.traverse((child) => {
          if (child.userData["pathType"] && child.userData["pathType"] !== "travel") {
            ...
            } else if (child instanceof import_Line2.Line2) {
              child.material.color = color;
            }
          }
        });
